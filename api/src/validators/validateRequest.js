...
let safeRegexURL = new RegExp(
  /^http[^\?]*.(jpg|jpeg|gif|png|tiff|bmp)(\?(.))?$/
);

let unsafeRegexURL = new RegExp(
  /^http[^\?]*.(jpg|jpeg|gif|png|tiff|bmp)(\?(.*))?$/
);
...
console.log(
  "isURLunsafe = ",
  validator.matches("http://hola.jpg", unsafeRegexURL)
);
...
console.log(
  "valid urls asdsa =",
  isValidURLImageValidator("https://www.hola.jpg")
);
...
const containsURLsRegex = new RegExp(
