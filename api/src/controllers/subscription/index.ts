...
export async function handleCreateNewSubscriptionRequest(
  ...
  try {
    ...
    const objToReturn = await subscriptionServices.handleNewSubscription(
      req.body,
      user_id
    );
...
export async function handleDeleteSubscriptionByIdRequest(
  ...
  try {
    ...
    const confirmationOfDeletion =
      await subscriptionServices.handleDeleteSubscription(
        subscription_id,
        user_id
      );
...
export async function handleUpdateSubscriptionByIdRequest(
  ...
  try {
    ...
    const confirmationOfUpdate =
      await subscriptionServices.handleUpdateSubscription(
        subscription_id,
        user_id,
        req.body
      );
...
export async function handleGetUserSubscriptionsRequest(
  ...
  try {
    ...
    const userSubscriptions =
      await subscriptionServices.handleGetUserSubscriptions(user_id);
